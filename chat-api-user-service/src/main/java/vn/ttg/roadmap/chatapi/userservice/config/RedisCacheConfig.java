 ...
 * 
 * <p>This configuration class sets up Redis as the cache backend for Spring Cache annotations
 * ({@code @Cacheable}, {@code @CachePut}, {@code @CacheEvict}). It provides:
 * 
 * <ul>
 *   <li>Base cache configuration with JSON serialization</li>
 *   <li>Per-cache TTL configuration from application properties</li>
 *   <li>Custom key generators for specific cache types</li>
 *   <li>Easy extensibility for adding new caches</li>
 * </ul>
 * 
 * <h3>Adding a New Cache</h3>
 * <p>To add a new cache, simply add its TTL to {@code application.yml}:
 * 
 * <pre>{@code
 * cache:
 *   ttl:
 *     oauth2-state-tokens: PT5M
 *     user-profiles: PT30M        # New cache - automatically configured!
 * }</pre>
 * 
 * <p>The cache will be automatically configured with the specified TTL. No code changes needed!
 * 
 * <h3>Key Features</h3>
 * <ul>
 *   <li><strong>JSON Serialization:</strong> Values are serialized as JSON for better compatibility</li>
 *   <li><strong>String Keys:</strong> Cache keys are stored as strings for readability</li>
 *   <li><strong>Null Safety:</strong> Null values are not cached (prevents cache pollution)</li>
 *   <li><strong>Per-Cache TTL:</strong> Each cache can have its own expiration time</li>
 *   <li><strong>Default TTL:</strong> Caches without explicit TTL use 5 minutes default</li>
 * </ul>
 * 
 * <h3>Usage Example</h3>
 * <pre>{@code
 ...
 *     @Cacheable(value = "user-profiles", keyGenerator = "defaultKeyGenerator")
